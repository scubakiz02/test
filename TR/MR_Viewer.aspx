...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <asp:Label ID="Label1" runat="server" Font-Bold="True" Font-Size="X-Large" Text="Maintenance Request Viewer"></asp:Label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<asp:Button ID="Button2" runat="server" Text="Grid View Status Board" BackColor="#FFFF99" PostBackUrl="~/TR/OpenTicketStatusBoard.aspx" /><br />
    <br />
        ...
        <ContentTemplate>
            <asp:Panel ID="Panel1" runat="server">
                ...
                <asp:SqlDataSource ID="TicketsSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ALTSConnectionString %>"
                    SelectCommand="SELECT T_ITR_Tickets.MR_Key AS Ticket#, T_IT_Tools.Department, T_IT_Tools.Tool, T_ITR_Tickets.Status, T_ITR_TicketNotes.Note, T_ITR_Tickets.IssueDate FROM T_ITR_Tickets INNER JOIN T_IT_Tools ON T_ITR_Tickets.Tool = T_IT_Tools.[Key] INNER JOIN T_ITR_TicketNotes ON T_ITR_Tickets.MR_Key = T_ITR_TicketNotes.MR_Key WHERE (T_ITR_Tickets.CloseDate IS NULL) AND (T_ITR_TicketNotes.NoteType = 'Org')">
                </asp:SqlDataSource>
                
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
                    ConnectionString="<%$ ConnectionStrings:ALTSConnectionString %>" 
                    SelectCommand="SELECT Tool FROM dbo.T_IT_Tools GROUP BY Tool ORDER BY Tool">
                </asp:SqlDataSource>
                
                <asp:SqlDataSource ID="SqlDataSource_SGN" runat="server" ConnectionString="<%$ ConnectionStrings:ALTSConnectionString %>" 
                    SelectCommand="SELECT T_Tool_SubGroup_Tag_Names.SG_Name, T_Tool_SubGroup_Tag_Names.SB_Tag FROM T_IT_Tools INNER JOIN T_Tool_SubGroup_Tag_Names ON T_IT_Tools.[Key] = T_Tool_SubGroup_Tag_Names.Tool_Key WHERE (T_IT_Tools.Tool = 'CMP 1')">
                </asp:SqlDataSource>
