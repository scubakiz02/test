...
Partial Class MR_OpenTicketStatusBoard
    ...
    Private Sub MR_OpenTicketStatusBoard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        For I = 0 To AreaRC - 1
            ...
            Dim DailyOrWeeklyChecklist As Boolean = If(SqlFunc = "[ALTS].[dbo].[fctn_DailyChecklistInfo]" OrElse SqlFunc = "[ALTS].[dbo].[fctn_WeeklyChecklistInfo]", True, False)
    ...
    Sub CreateRecord(AreaKey As Integer)
        ...
        Try
            ...
            My_DR("Shift") = SatiCode.GetMyDataSet("SELECT Shift FROM [ALTS].[dbo].[fctn_GetShift]()").Tables(0).Rows(0)("Shift")
    ...
    Sub Build(AreaKey As Integer)
        ...
        DS = SatiCode.GetMyDataSet("SELECT A.Area, I.Interval, A.Assignee, D.[Key], D.AreaKey, D.Operator, Sql.LogStatus, Sql.StripeColor, MAX(D.Date) AS MaxDate FROM [ALTS].[dbo].[T_LogData] D INNER JOIN [ALTS].[dbo].[T_LogArea] A ON D.AreaKey = A.[Key] INNER JOIN [ALTS].[dbo].[T_LogAreaInterval] I ON A.IntervalKey=I.[Key] CROSS APPLY [ALTS].[dbo].[fctn_ChecklistRecordInfo]((SELECT [Key] FROM [ALTS].[dbo].[T_LogData] WHERE AreaKey=" & AreaKey & " AND CAST(Date As Date) = '" & CurrLogDate & "'), 1, '" & CurrLogDate & "') Sql WHERE CAST(D.Date As Date) = '" & CurrLogDate & "' AND AreaKey=" & AreaKey & " GROUP BY A.Area, I.Interval, A.Assignee, D.[Key], D.AreaKey, D.Operator, Sql.LogStatus, Sql.StripeColor")
    ...
    Protected Sub RedirectToLogAspx(sender As Object, e As EventArgs)
        Response.Redirect("/DBMaintenance/DevTest/LoggingTemplates/Log.aspx?Key=" & sender.ID & "&WHERE=" & Request.QueryString("WHERE") & "&Department=" & Request.QueryString("Department") & "&View=" & Request.QueryString("View"))
    End Sub

    Protected Sub DepartmentMenu_onClick(sender As Object, e As EventArgs)
        Response.Redirect(Request.FilePath.ToString & "?WHERE=" & Request.QueryString("WHERE") & "&Department=" & sender.Text & "&View=" & Request.QueryString("View"))
    End Sub

    Protected Sub ViewMenu_onClick(sender As Object, e As EventArgs)
        Response.Redirect(Request.FilePath.ToString & "?WHERE=" & Request.QueryString("WHERE") & "&Department=" & Request.QueryString("Department") & "&View=" & sender.Text)
    End Sub
