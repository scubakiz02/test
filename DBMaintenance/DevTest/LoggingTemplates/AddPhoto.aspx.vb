...
Partial Class MR_OpenTicketStatusBoard
    ...
    Private Sub MR_OpenTicketStatusBoard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        DR = SatiCode.GetMyDataSet("SELECT A.[Key], I.SqlFunc2ndArg, D.Date, A.Area, I.SqlFunc FROM [SatiTest].[dbo].[T_LogData] D INNER JOIN [SatiTest].[dbo].[T_LogArea] A ON D.AreaKey=A.[Key] INNER JOIN [SatiTest].[dbo].[T_LogAreaInterval] I ON A.IntervalKey=I.[Key] WHERE D.[Key]=" & DataKeyFromQueryString).Tables(0).Rows(0)
    ...
    Protected Sub ExitIframeButton_onClick(sender As Object, e As EventArgs)
        ...
        Try
            If sender.Text = "Set" Then
                ...
                Else
                    DuplicateDS = SatiCode.GetMyDataSet("SELECT PhotoTitle FROM [SatiTest].[dbo].[T_LogDataPhotos] P WHERE DataKey=" & DataKeyFromQueryString)
                    ...
                    'ExecuteSqlQuery("INSERT INTO [SatiTest].[dbo].[T_LogDataPhotos] (DataKey, PhotoTitle, PhotoFilePath) VALUES (" & DataKeyFromQueryString & ", '" & Title & "', '" & Path.Combine(uploadDirectory, FileName) & "')")
                    My.Computer.FileSystem.RenameFile(CompleteImagePath, FileName)
                    ExecuteSqlQuery("INSERT INTO [SatiTest].[dbo].[T_LogDataPhotos] (DataKey, PhotoTitle, PhotoFilePath, ContentType, FileName) VALUES (" & DataKeyFromQueryString & ", '" & SqlProofSingleQuotes(UserInput) & "', '" & Path.Combine(uploadDirectory, FileName) & "', '" & Session("ContentType") & "', '" & FileName & "')")
                End If
            Else 'Cancel
