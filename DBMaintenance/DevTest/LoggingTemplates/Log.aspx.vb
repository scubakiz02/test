...
Partial Class MR_OpenTicketStatusBoard
    ...
    Dim WebpageUrl As String = "/DBMaintenance/DevTest/LoggingTemplates/Log.aspx"
    ...
    Private Sub MR_OpenTicketStatusBoard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If KeyFromQueryString IsNot Nothing Then 'if this is true, displaying Log.aspx for operator to fill out
            ...
            DS = SatiCode.GetMyDataSet("SELECT TOP (100) A.[Key] As AreaKey, Area, I.SqlFunc, I.SqlFunc2ndArg, L.Label As Label, L.[Key] As LabelKey, L.Range As Range, L.TbxOverlay, L.CheckboxOverTextbox, U.Unit From [SatiTest].[dbo].[T_LogArea] A INNER JOIN [SatiTest].[dbo].[T_LogData] D ON A.[Key]=D.AreaKey INNER JOIN [SatiTest].[dbo].[T_LogLabel] L ON A.[Key]=L.AreaKey LEFT JOIN [SatiTest].[dbo].[T_LogUnit] U ON L.UnitKey=U.[Key] INNER JOIN [SatiTest].[dbo].[T_LogAreaInterval] I ON A.IntervalKey=I.[Key] WHERE D.[Key]=" & KeyFromQueryString & " ORDER BY L.LabelOrder")
            RC = DS.Tables(0).Rows.Count

            MostRecentRec = "SELECT Top(1) * FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString & " ORDER BY Date DESC"

            CommentSqlDataSource.SelectCommand = "SELECT OpComments.[Key], OpComments.Comment FROM [SatiTest].[dbo].[T_LogOperatorComments] OpComments WHERE OpComments.CommentKey=(Select Top(1) [Key] FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString & " ORDER BY Date DESC)"
            CommentGridView.DataBind()
            If CommentGridView.Rows.Count > 0 Then
                CommentGridView.Visible = True
            End If

            ShiftSqlDataSource.SelectCommand = "SELECT Top(1) Shift FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString & " ORDER BY Date Desc"
            ...
            LogDS = SatiCode.GetMyDataSet("SELECT Top(1) * From [SatiTest].[dbo].[T_LogData] D CROSS APPLY (SELECT * FROM " & DR1("SqlFunc") & "(" & AreaFromQueryString & ", " & DR1("SqlFunc2ndArg") & ", '" & GetSingleDbField("SELECT Date FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString, "Date") & "')) DA WHERE [Key]=" & KeyFromQueryString & " ORDER BY Date DESC")
            ...
            PhotoDS = SatiCode.GetMyDataSet("SELECT * FROM [SatiTest].[dbo].[T_LogDataPhotos] WHERE DataKey=" & KeyFromQueryString)
        ...
        ElseIf AreaFromQueryString IsNot Nothing Then  'if this is true, displaying webpage in iframe within ChecklistBuilder.aspx
            DS = SatiCode.GetMyDataSet("SELECT TOP (100) Area, L.Label As Label, L.[Key] As LabelKey, L.Range As Range, L.TbxOverlay, L.CheckboxOverTextbox, U.Unit From [SatiTest].[dbo].[T_LogArea] A INNER JOIN [SatiTest].[dbo].[T_LogLabel] L ON A.[Key]=L.AreaKey LEFT JOIN [SatiTest].[dbo].[T_LogUnit] U ON L.UnitKey=U.[Key] WHERE A.[Key]=" & AreaFromQueryString & " ORDER BY L.LabelOrder")
            RC = DS.Tables(0).Rows.Count

            TitleLabel.Text = GetSingleDbField("SELECT Area FROM [SatiTest].[dbo].[T_LogArea] WHERE [Key]=" & AreaFromQueryString, "Area")
                ...
                If TypeOf ctrl Is TextBox Then
                    ...
                    If Request.QueryString("Key") IsNot Nothing Then 'this means user is logging inputs
                        ...
                        Catch ex As Exception
                            ...
                            My_DS2 = SatiCode.GetMyDataSet("SELECT [Key] FROM [SatiTest].[dbo].[T_LogLabel] WHERE AreaKey=" & AreaFromQueryString)
                            ...
                            ExecuteSqlQuery("UPDATE [SatiTest].[dbo].[T_LogData] SET Inputs='" & JsonSerializer.Serialize(Session("LabelInputMap")) & "', OutOfRange='" & JsonSerializer.Serialize(LabelOutOfRangeMap) & "' WHERE [Key]=" & KeyFromQueryString)
                    ...
                    If TbxOverlay IsNot Nothing AndAlso MalleableCtrl.Attributes(TbxOverlay) IsNot Nothing Then 'if TbxOverlay is null, it is a standard textbox
                        ...
                        If Request.QueryString("Key") IsNot Nothing Then 'this means user is logging inputs
                            ...
                            ElseIf TbxOverlay = "OffHandAuto" Then
                                ...
                                If Not OffHandAutoValue.Contains("Select") Then
    ...
    Function ValidateInput(ControlID As String, Value As String) As Boolean
        ...
        For Each ctrl As Control In Pnl.Controls
            ...
            If TypeOf ctrl Is TextBox Then
                ...
                TbxOverlay = GetSingleDbField("SELECT TbxOverlay FROM [SatiTest].[dbo].[T_LogLabel] WHERE [Key]=" & LabelKey, "TbxOverlay")
    ...
    Protected Sub DbUploadTimer_Tick(sender As Object, e As EventArgs)
        ...
        Else
            If Not IsDBNull(GetSingleDbField("SELECT Top(1) * FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & Request.QueryString("Key") & " ORDER BY Date DESC", "Operator")) Then 'only IF Operator is NOT null
    ...
    Protected Sub UpdateButton_onClick(sender As Object, e As EventArgs)
        ShiftSqlDataSource.UpdateCommand = "UPDATE [SatiTest].[dbo].[T_LogData] SET [Shift] ='" & sender.Parent.FindControl("ShiftDropDownList").SelectedValue & "' WHERE [Key]=(SELECT Top(1) [Key] FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString & " ORDER BY Date Desc)"
    ...
    Function ModifyInput(ID As String, Value As String) As Boolean
        ...
        Try 'in case user in on last input, in which case sql will return 'There is no row at position 0.'
            ExecuteSqlQuery("UPDATE [SatiTest].[dbo].[T_LogData] SET KeyOfLastLabel=" & SatiCode.GetMyDataSet("SELECT TOP(1) [Key], AreaKey, Label, LabelOrder FROM [SatiTest].[dbo].[T_LogLabel] WHERE AreaKey=(SELECT AreaKey FROM [SatiTest].[dbo].[T_LogLabel] WHERE [Key]= " & LabelKey & ") AND LabelOrder > (SELECT LabelOrder FROM [SatiTest].[dbo].[T_LogLabel] WHERE [Key]= " & LabelKey & ") ORDER BY LabelOrder").Tables(0).Rows(0)("Key") & " WHERE [Key]=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key")) 'update KeyOfLastLabel field in DB
        Catch ex As Exception
    ...
    Protected Sub DoneButton_Click(sender As Object, e As EventArgs)
        ...
        Dim NumOfNotes As Integer = SatiCode.GetMyDataSet("Select Count([Key]) As NumOfNotes FROM [SatiTest].[dbo].[T_LogOperatorComments] WHERE CommentKey=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key")).Tables(0).Rows(0)("NumOfNotes")
    ...
    Sub MarkAsDone()
        ...
        DS = SatiCode.GetMyDataSet("SELECT [Key], Range FROM [SatiTest].[dbo].[T_LogLabel] WHERE AreaKey=" & GetSingleDbField("SELECT AreaKey FROM [SatiTest].[dbo].[T_LogData] WHERE [Key]=" & KeyFromQueryString, "AreaKey"))
        ...
        ExecuteSqlQuery("UPDATE [SatiTest].[dbo].[T_LogData] SET Ranges='" & JsonSerializer.Serialize(LabelRangeMap) & "', CompleteLog=1, Date='" & System.DateTime.Now.ToString() & "' WHERE [Key]=" & KeyFromQueryString) 'record to 'Ranges' field in T_LogData
        Update_All_InputsValid_Field()
    ...
    Sub BuildDynamicAsp()
        If Request.QueryString("Key") IsNot Nothing Then 'if true, user is filling out a log sheet
            DS = SatiCode.GetMyDataSet("Select T.Title, L.[Key] As ID, S.[Key] As StampedRecordKey, S2.StampedBy As StampedBy, T.RoleID FROM [SatiTest].[dbo].[T_LogStamp] S RIGHT JOIN [SatiTest].[dbo].[T_LogStampList] L On L.[Key]=S.StampKey And DataRecordKey=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key") & " INNER JOIN [SatiTest].[dbo].[T_LogStampTitle] T On L.TitleKey=T.[Key] LEFT JOIN [SatiTest].[dbo].[T_LogStamp] S2 On S.[Key]=S2.[Key] WHERE AreaKey=" & AreaFromQueryString & " AND Active=1")
        Else 'user is in ChecklistBuilder.aspx editing or creating a checklist
            DS = SatiCode.GetMyDataSet("SELECT Stamped.Title, Stamp.[Key] As ID FROM [SatiTest].[dbo].[T_LogStampList] Stamp INNER JOIN [SatiTest].[dbo].[T_LogStampTitle] Stamped ON Stamp.TitleKey=Stamped.[Key] WHERE Active=1 AND AreaKey=" & AreaFromQueryString)
        End If
        ...
        For I = 0 To DRC.Count - 1
            ...
            If Request.QueryString("Key") IsNot Nothing Then 'if true, user is filling out a log sheet
                ...
                If Button.Text = "Stamp" AndAlso GetSingleDbField("SELECT COUNT(RoleName) As ContainsRole FROM [SatiUsers].[dbo].aspnet_UsersInRoles INNER JOIN [SatiUsers].[dbo].aspnet_Users On [SatiUsers].[dbo].aspnet_UsersInRoles.UserId = [SatiUsers].[dbo].aspnet_Users.UserId INNER JOIN [SatiUsers].[dbo].aspnet_Roles On [SatiUsers].[dbo].aspnet_UsersInRoles.RoleId = [SatiUsers].[dbo].aspnet_Roles.RoleId INNER JOIN [SatiTest].[dbo].[T_LogStampList] On [SatiUsers].[dbo].aspnet_Roles.RoleId='" & DR("RoleID") & "' WHERE [SatiTest].[dbo].[T_LogStampList].[Key]=" & Button.ID.Split("_")(1) & " And [SatiUsers].[dbo].aspnet_Users.UserName = '" & User.Identity.Name.ToString & "'", "ContainsRole") Then
        ...
        DS = SatiCode.GetMyDataSet("SELECT Comment FROM [SatiTest].[dbo].[T_LogCommentList] WHERE AreaKey=" & AreaFromQueryString & "ORDER BY CommentOrder")
        DRC = DS.Tables(0).Rows
    ...
    Protected Sub BackToStatusBoard_OnClick(sender As Object, e As EventArgs)
        Response.Redirect("/DBMaintenance/DevTest/LoggingTemplates/StatusBoard.aspx?WHERE=" & Request.QueryString("WHERE") & "&Department=" & Request.QueryString("Department") & "&View=" & Request.QueryString("View"))
    End Sub
    ...
    Protected Sub AddPhotoButton_OnClick(sender As Object, e As EventArgs)
        ...
        PreviewPanel_iframe.Attributes.Add("src", "/DBMaintenance/DevTest/LoggingTemplates/AddPhoto.aspx" & "?" & Request.RawUrl.Split("?")(1) & "&DataKey=" & KeyFromQueryString)
    End Sub
