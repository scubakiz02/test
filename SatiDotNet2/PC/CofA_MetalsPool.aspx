<%@ Page Title="" Language="VB" MasterPageFile="~/MasterPage1.master" AutoEventWireup="false" CodeFile="CofA_MetalsPool.aspx.vb" Inherits="PC_CofA_MetalsPool" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
 <asp:Label ID="Label1" runat="server" Font-Bold="True" Font-Size="X-Large" Text="View 200mm Metals pool for CofA"></asp:Label><br />
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
        <ContentTemplate>
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ALTSConnectionString %>" SelectCommand="SELECT [Date/Time], Source, [Test Type], Idenyification, Location, ROUND(Ca, 3) AS Ca, ROUND(Ma, 3) AS Mg, ROUND(Ni, 3) AS Ni, ROUND(Zn, 3) AS Zn, ROUND(Al, 3) AS Al, ROUND(Fe, 3) AS Fe, ROUND(Cr, 3) AS Cr, ROUND(Cu, 3) AS Cu, ROUND(Na, 3) AS Na, ROUND(K, 3) AS K, Notes FROM [GFAAS Data] WHERE (Source LIKE N'SC%') AND ([Date/Time] &gt; DATEADD(year, - 1, GETDATE())) AND (Notes IS NULL) ORDER BY [Date/Time] DESC"></asp:SqlDataSource>
        </ContentTemplate>
    </asp:UpdatePanel>
</asp:Content>
