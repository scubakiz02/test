...
Partial Class MR_OpenTicketStatusBoard
    ...
    Private Sub MR_OpenTicketStatusBoard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            AreaDropDownList_SqlDataSource.SelectCommand = "SELECT A.Area, A.[Key] FROM [ALTS].[dbo].[T_LogArea] A LEFT JOIN [ALTS].[dbo].[T_LogAreaInterval] I ON A.IntervalKey=I.[Key] WHERE " & If(Session("AreaInterval") Is Nothing OrElse Session("AreaInterval") = "All", String.Empty, " (A.IntervalKey=" & Session("AreaInterval") & " OR DATEDIFF(DAY, A.DateCreated, GETDATE()) = 0) AND") & " OneTimeDate IS NULL OR (OneTimeDate IS NOT NULL AND ((SELECT CompleteLog FROM [ALTS].[dbo].[T_LogData] WHERE AreaKey=A.[Key])=0 OR (SELECT CompleteLog FROM [ALTS].[dbo].[T_LogData] WHERE AreaKey=A.[Key]) IS NULL)) ORDER BY A.Area"
    ...
    Function ValidateInput(Pnl As Panel) As Boolean
        ...
        For Each ctrl As Control In Pnl.Controls
            ...
            If TypeOf ctrl Is TextBox Then
                ...
                CheckboxOverTextbox = If(IsDBNull(SatiCode.GetMyDataSet("SELECT CheckboxOverTextbox FROM [ALTS].[dbo].[T_LogLabel] WHERE [Key]=" & TextBox.ID).Tables(0).Rows(0)("CheckboxOverTextbox")), Nothing, SatiCode.GetMyDataSet("SELECT CheckboxOverTextbox FROM [ALTS].[dbo].[T_LogLabel] WHERE [Key]=" & TextBox.ID).Tables(0).Rows(0)("CheckboxOverTextbox"))
    ...
    Protected Sub TextBox_OnTextChanged(sender As Object, e As EventArgs)
        ...
        Try
            ExecuteSqlQuery("UPDATE [ALTS].[dbo].[T_LogData] Set KeyOfLastLabel=" & SatiCode.GetMyDataSet("Select TOP(1) [Key] FROM [ALTS].[dbo].[T_LogLabel] WHERE LabelOrder > (Select LabelOrder FROM [ALTS].[dbo].[T_LogLabel] WHERE [Key]=" & sender.ID & ")").Tables(0).Rows(0)("Key") & " WHERE [Key]=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key")) 'update KeyOfLastLabel field in DB
        Catch ex As Exception
        End Try
    ...
    '    Dim NumOfComments As Integer = SatiCode.GetMyDataSet("Select Count([Key]) As NumOfComments FROM [ALTS].[dbo].[T_LogOperatorComments] WHERE CommentKey=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key")).Tables(0).Rows(0)("NumOfComments")
    ...
    Sub MarkAsDone()
        ExecuteSqlQuery("UPDATE [ALTS].[dbo].[T_LogData] SET CompleteLog=1 WHERE [Key]=" & SatiCode.GetMyDataSet(MostRecentRec).Tables(0).Rows(0)("Key"))
        Update_All_InputsValid_Field()
