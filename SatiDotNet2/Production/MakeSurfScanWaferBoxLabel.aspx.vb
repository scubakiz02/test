...
Partial Class Production_MakeSurfScanWaferBoxLabel
    Inherits System.Web.UI.Page
    Dim SatiCode As New Class1
    Public DA_Partical_Data_From_Tool As New System.Data.SqlClient.SqlDataAdapter
    Dim DS_Partical_Data_From_Tool As New Data.DataSet
    Sub ReadyCheck()
        Try
            If Not Me.ListBoxTool.SelectedItem.ToString = "" And Not ListBoxStation.SelectedItem.ToString = "" And Not DropDownListPrinter.SelectedItem.ToString = "Select Printer..." Then
                'Me.PanelScribeCheck.Visible = True
                Me.PanelScribeCheck.Visible = False
                Me.Button_Pull_Data.Visible = True
            Else
                Me.PanelScribeCheck.Visible = False
                Me.Button_Pull_Data.Visible = False
            End If
    ...
    Protected Sub ListBoxTool_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBoxTool.SelectedIndexChanged
        ReadyCheck()
    End Sub

    Protected Sub ListBoxStation_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBoxStation.SelectedIndexChanged
        ReadyCheck()
    End Sub

    Protected Sub DropDownListPrinter_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownListPrinter.SelectedIndexChanged
        ReadyCheck()
    End Sub

    Protected Sub Button_Pull_Data_Click(sender As Object, e As EventArgs) Handles Button_Pull_Data.Click
        Dim Tool As String = Me.ListBoxTool.SelectedValue.ToString
        Dim Station As String = Me.ListBoxStation.SelectedItem.ToString
        Dim DR As Data.DataRow
        Dim I As Integer = 0
        Dim Instance As String = ""
        Dim I_Slot As Integer = 0
        Dim S_Slot As Integer = 30
        Dim Scribe As String = ""
        Dim DS_Scribe As New Data.DataSet
        ...
        DS_Partical_Data_From_Tool = SatiCode.GetMyDataSetAutoData("SELECT TOP (50) Comment2 AS Instance#, SourceSlotID AS I_Slot, DestinationStationID AS Station, DestinationSlotID AS S_Slot, ID#, RUN#, Wafer_log, Comment1 FROM dbo.SP1_Data WHERE (Machine = N'" & Tool & "') AND (DestinationStationID = " & Station & ") ORDER BY Entry DESC")
        'Me.GridViewLook.DataSource = DS_Partical_Data_From_Tool
        'Me.GridViewLook.DataBind()


        If Not DS_Partical_Data_From_Tool.Tables(0).Rows.Count > 0 Then
            Me.TextBoxInfo.Text = "Zero Records Found"
            Exit Sub
        End If
        ...
        DR = DS_Partical_Data_From_Tool.Tables(0).Rows(0)

        'LabelWhat.Text = "Station " & Me.ListBoxStation.SelectedItem.ToString & " From " & Me.ListBoxTool.SelectedItem.ToString & " With Lot# " & DR("ID#") & "-" & DR("RUN#") & "-" & DR("Wafer_log") & "."
        LabelWhat.Text = "Entered Lot# " & DR("ID#") & "-" & DR("RUN#") & "-" & DR("Wafer_log") & "."
        For I = 0 To DS_Partical_Data_From_Tool.Tables(0).Rows.Count - 1
            DR = DS_Partical_Data_From_Tool.Tables(0).Rows(I)
            If DR("S_Slot") <= S_Slot Then
                If Not DR("S_Slot") = S_Slot Then

                    DS_Scribe.Clear()

                    With SelectCmd
                        .CommandText = "SELECT dbo.T7_WaferActionTracking.T7 FROM dbo.T7_WaferActionTracking INNER JOIN dbo.T7_InstanceInfo ON dbo.T7_WaferActionTracking.WAT_Key = dbo.T7_InstanceInfo.WAT_Key WHERE (dbo.T7_InstanceInfo.InstanceID = " & DR("Instance#") & ") AND (dbo.T7_InstanceInfo.Slot = " & DR("I_Slot") & ")"
                        .Connection = Connection
                    End With
                    DA_Scribe.SelectCommand = SelectCmd


                    DA_Scribe.Fill(DS_Scribe)

                    If DS_Scribe.Tables(0).Rows.Count = 1 Then
                        DR_Scribe = DS_Scribe.Tables(0).Rows(0)
                        Scribe = DR_Scribe("T7")
                    Else
                        Scribe = "No Scribe"
                        CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & DR("S_Slot")), TextBox).BackColor = Drawing.Color.Red
                    End If

                    CType(Me.UpdatePanel1.FindControl("LabelInstance" & DR("S_Slot")), Label).Text = DR("Instance#")
                    CType(Me.UpdatePanel1.FindControl("LabelSlot" & DR("S_Slot")), Label).Text = DR("S_Slot")
                    CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & DR("S_Slot")), TextBox).Text = Scribe
                    CType(Me.UpdatePanel1.FindControl("LabelScribe" & DR("S_Slot")), Label).Text = Scribe

                    If Not Instance = DR("Instance#") Then
                        CType(Me.UpdatePanel1.FindControl("LabelInstance" & DR("S_Slot")), Label).BackColor = Drawing.Color.LemonChiffon
                        CType(Me.UpdatePanel1.FindControl("LabelSlot" & DR("S_Slot")), Label).BackColor = Drawing.Color.LemonChiffon
                        CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & DR("S_Slot")), TextBox).BackColor = Drawing.Color.LemonChiffon
                    End If
                End If
    ...
    Sub Tog(Reset_or_Strip As String)
        Dim I As Integer = 0
        Reset_or_Strip = UCase(Reset_or_Strip)
        Select Case Reset_or_Strip
            Case "RESET"
                For I = 1 To 25
                    CType(Me.UpdatePanel1.FindControl("LabelInstance" & I), Label).Text = 0
                    CType(Me.UpdatePanel1.FindControl("LabelSlot" & I), Label).Text = 0
                    CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text = ""
                    CType(Me.UpdatePanel1.FindControl("LabelScribe" & I), Label).Text = ""

                    CType(Me.UpdatePanel1.FindControl("LabelInstance" & I), Label).Visible = True
                    CType(Me.UpdatePanel1.FindControl("LabelSlot" & I), Label).Visible = True
                    CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Visible = True
                    'CType(Me.UpdatePanel1.FindControl("LabelScribe" & I), Label).Visible = True

                    CType(Me.UpdatePanel1.FindControl("LabelInstance" & I), Label).BackColor = Drawing.Color.Transparent
                    CType(Me.UpdatePanel1.FindControl("LabelSlot" & I), Label).BackColor = Drawing.Color.Transparent
                    CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).BackColor = Drawing.Color.Transparent
                Next
            Case "STRIP"
                For I = 1 To 25
                    If CType(Me.UpdatePanel1.FindControl("LabelInstance" & I), Label).BackColor = Drawing.Color.LemonChiffon Then
                        If CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text.Length >= 10 Then
                            CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text = Left(CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text, CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text.Length - 4)
                        End If
    ...
    Sub ShowPrintButton()
        Dim I As Integer = 0
        For I = 1 To 25
            If UCase(CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text) = UCase(CType(Me.UpdatePanel1.FindControl("LabelScribe" & I), Label).Text) Then
                CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).BackColor = Drawing.Color.Transparent
            Else
                CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).BackColor = Drawing.Color.LemonChiffon
            End If
        Next

        For I = 1 To 25
            If Not CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).BackColor = Drawing.Color.Transparent Then
    ...
    Protected Sub TextBoxScribe1_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe1.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe2_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe2.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe3_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe3.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe4_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe4.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe5_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe5.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe6_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe6.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe7_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe7.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe8_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe8.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe9_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe9.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe10_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe10.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe11_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe11.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe12_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe12.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe13_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe13.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe14_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe14.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe15_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe15.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe16_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe16.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe17_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe17.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe18_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe18.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe19_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe19.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe20_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe20.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe21_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe21.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe22_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe22.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe23_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe23.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe24_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe24.TextChanged
        ShowPrintButton()
    End Sub
    Protected Sub TextBoxScribe25_TextChanged(sender As Object, e As EventArgs) Handles TextBoxScribe25.TextChanged
        ShowPrintButton()
    End Sub
    ...
    Protected Sub ButtonPrint_Click(sender As Object, e As EventArgs) Handles ButtonPrint.Click
        ...
        If Not Me.DropDownListPrinter.SelectedItem.Text = "Select Printer..." Then
            Printer = "\\PWI-40\" & Me.DropDownListPrinter.SelectedItem.Text
        Else
            Exit Sub
        End If

        GoSPxSQL(Me.ListBoxTool.SelectedValue, Me.ListBoxStation.SelectedValue, Printer)
    ...
    Sub GoSPxSQL(ByVal tool As String, ByVal Station As String, ByVal Printer As String)
        ...
        MailStringBuild = MailStringBuild + "Tool: " & Me.ListBoxTool.SelectedItem.Text & Chr(13)
        MailStringBuild = MailStringBuild + "Station: " & Me.ListBoxStation.SelectedItem.Text & Chr(13)
        ...
        For i = 0 To SP1RowCount - 1
            ...
            If Slot < LastSlot Then
                If Not Slot = LastSlot Then
                    ...
                    DR_Partical("RFID_1") = DR_SP1("RFID_1")
                    ...
                    If T7Ver = False Then
                        ...
                        If TestT7(DR_WatP("T7")) = False Then
                            Connection.Close()
                            AutoDataConnection.Close()
                            Me.TextBoxInfo.Text = "Scribe not correct"
                            Exit Sub
                        End If
        ...
        If SatiCode.IsLotNumberReal(DR_SP1("ID#") & "-" & DR_SP1("RUN#") & "-" & DR_SP1("Wafer_log")) = False Then
            MakeLabel = False
            Me.TextBoxInfo.Text = "The Lot Number Is Not Valid..... You Will Need To Make A Instance And Rerun With The Correct Lot Number."
        ...
        If Not CheckT7Requirement = "Good" Then
            Me.TextBoxInfo.Text = CheckT7Requirement & ". Rerun under Instance Number " & IKey & " and remove bad wafer."
            MakeLabel = False
        End If
        ...
        If SatiCode.IS_RFID_Enable(LabelID) = True Then
            ...
            If CustomerName.Contains("") Then 'IBM

            End If

            Select Case CustomerName
                Case CustomerName.Contains("")
            ...
            If SatiCode.Check_RFID_Used_Last20Days(MyRFID, SPxSlot1CreationDate) = True Then
                ...
                Me.TextBoxInfo.Text = "Re Run Label Was Made"
            End If
        ...
        If LabelID = "3265" Then
                ...
                For ImGoingToWait As Int16 = 0 To 5
                    ...
                    Select Case SatiCode.RFID_WriteTable("Look", IKey, tool, Station, RFIDString)
                        ...
                        Case "No Record Found"
                            Me.TextBoxInfo.Text = "No Record Found In The RFID DB Table"
                    End Select
            ...
            Else
                Me.TextBoxInfo.Text = "Could Not Write To RFID DB Table"
            End If
        End If
        ...
        If MakeLabel = True Then
            ...
            Me.TextBoxInfo.Text = "Label Was Made" & Chr(13) & File
        End If
    ...
    Function TestT7(ByVal T7 As String) As Boolean
        T7 = UCase(T7)
        'FInd last slot on page
        Dim I As Integer = 25
        For I = 25 To 1 Step -1
            If Not UCase(CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text) = "" Then
                If T7 = UCase(CType(Me.UpdatePanel1.FindControl("TextBoxScribe" & I), TextBox).Text) Then
                    Return True
                Else
                    Return False
                End If
    ...
    Private Sub Production_MakeSurfScanWaferBoxLabel_Load(sender As Object, e As EventArgs) Handles Me.Load
